...
function parseHSL(hslStr) {
  ...
  if (!match) return { h: 43, s: 75, l: 55 };
...
const BorderGlow = forwardRef(({
  ...
  const handlePointerMove = useCallback((e) => {
    // Disable hover-glow interactivity on mobile/touch screens
  ...
  useEffect(() => {
    ...
    if (isMobile) {
      // Mobile auto-glow mode: rotates contour light continuously
      card.classList.add('auto-glow-active');
      ...
      const tick = () => {
        if (!active) return;
        angle = (angle + 1) % 360; // Rotate 1 degree per frame
  ...
  return (
    ...
    >
      ...
      <div className="border-glow-inner" style={{ width: '100%', height: '100%', display: 'block', overflow: 'visible' }}>
        {children}
      </div>
    </Component>
  );
